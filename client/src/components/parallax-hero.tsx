...
export default function ParallaxHero({ lowestPrice, onInfoClick }: ParallaxHeroProps) {
  ...
  useEffect(() => {
    ...
    if (isMobile && typeof DeviceOrientationEvent !== "undefined") {
      ...
      attachGyro();
      document.addEventListener("touchstart", attachGyro, { once: true });
      return () => {
        window.removeEventListener("deviceorientation", handleOrientation);
        document.removeEventListener("touchstart", attachGyro);
      };
    } else {
  ...
  return (
    <section className="relative overflow-hidden bg-[#0a0a0c]">
      {/* ── Hero image — moves with mouse/gyro ── */}
      <motion.div
      ...
      <div
        ...
        style={{
          background: "linear-gradient(to top, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.65) 38%, rgba(0,0,0,0.18) 68%, transparent 100%)",
        }}
      />
      <div
        className="absolute inset-0 pointer-events-none"
      ...
      />
      {/* Subtle brand-cyan glow */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: "radial-gradient(ellipse 70% 55% at 65% 50%, rgba(20,180,180,0.09) 0%, transparent 70%)",
        }}
      />
